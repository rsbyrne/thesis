...
dims = dict(
    f=tuple(val / 20 for val in range(20, 5, -1)),
    aspect=tuple(2.**(val / 10) for val in range(0, 11)),
    H=tuple(10.**(val / 10) for val in range(-10, 11)),
    )
constants = dict(
    flux=0.,
    )
...
with planetengine.campaign.Job(*(dims[key] for key in sorted(dims))) as job:
    ...
    planetengine.set_global_anchor(job.campaignname, '.')
    ...
    datafilepath = job.logfilepath.with_suffix('data')

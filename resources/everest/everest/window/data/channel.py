...
class DataChannel:
    ...
    def _preprocess(cls, data, **kwargs):
        data = _np.array(data).flatten()
        dtype = data.dtype
        if _np.issubdtype(dtype, _np.datetime64):
            subcl = cls.Datetime
        elif _np.issubdtype(dtype, _np.dtype(str).type):
            subcl = cls.Various
    ...
    class _Data:
        ...
        def auto_axis_configs(self, nTicks = 5):
            ...
            lims = (_np.min(tickVals), _np.max(tickVals))
            label = self.label
            if len(suffix):
                label += f" ({suffix})"
    ...
    class Orderable(_Data):
        ...
        def _process_caps(self, tickVals):
            ...
            if lCap:
                tickVals = _np.delete(
                    tickVals,
                    _np.argwhere(tickVals <= lLim).flatten()
                    )
                tickVals = _np.insert(tickVals, 0, lLim)
            if uCap:
                tickVals = _np.delete(
                    tickVals,
                    _np.argwhere(tickVals >= uLim).flatten()
                    )
                tickVals = _np.append(tickVals, uLim)
            return tickVals
        ...
        def _merge_self_other(self,
            ...
            return type(self)(
                _np.concatenate([selfdata, otherdata]),
                lims = (minLim, maxLim),
    ...
    class Numeric(Orderable):
        ...
        def __init__(self,
            ...
            if log and not islog:
                ...
                if label:
                    label = maths_label(r"\log_{10}\;") + label
            else:
        ...
        def nice_interval(self, nTicks, bases = frozenset({1, 2, 5})):
            llim, ulim = self.lims
            valRange = ulim - llim
        ...
        def shorten_label(cls, val):
            ...
            if signo == 1:
                return maths_label(r'10^{' + str(exp) + '}')
            if signo % 0.1:
                return maths_label(''.join((
        ...
        def nice_tickLabels(cls, tickVals):
            maxLog10 = math.log10(_np.max(_np.abs(tickVals)))
            adjPower = round(math.floor(maxLog10) / 3.) * 3
            if abs(adjPower) > 0:
                suffix = r'\times10^{' + str(adjPower) + '}'
                tickVals = _np.round(tickVals * 10. ** -adjPower, 12)
            else:
                suffix = ''
            ...
            if len(tickLabels) > 5:
                ...
                if diffs[-1] < minProx:
                    tickLabels[-2] = ''
            return [maths_label(lb) for lb in tickLabels], maths_label(suffix)

        def nice_tickVals(self, nTicks, bases = frozenset({1, 2, 5}), origin = 0.):
            llim, ulim = self.lims
            if llim == ulim:
                return _np.array(llim), []
            nTicks = max(2, round(nTicks))
            tickValsChoices = []
            for nT in (nTicks + off for off in range(-3, 4)):
                if nT < 1:
                    continue
...
#                 nTicks = int((uLim - lLim) / step) + 1
        ...
        def nice_minorTickVals(tickVals):
            ...
            return _np.linspace(tickVals[0], tickVals[-1], nTicks)
            # return tickVals
        ...
        def proc_log_label(cls, val):
            if val % 1:
                ...
                if (rnd := round(coeff)) == coeff:
                    ...
                    return maths_label(str(rnd) + r'\times10^{' + str(floorexp) + '}')
                out = r'10^{' + f"{val:.2}" + '}'
                if not val % 0.01:
                    return maths_label(out)
                return maths_label(r'\sim' + out)
            if -2 <= val <= 3:
                return maths_label(str(round(10**val, round(abs(val)))))
            return maths_label(r'10^{' + str(val) + r'}')
        ...
        def nice_ticks(self, nTicks):
            ...
            tickVals = _np.round(tickVals, 12)
    ...
    class Datetime(Orderable):
        ...
        def __init__(self,
            ...
            super().__init__(data, **kwargs)
            lims = (
                (_np.datetime64(lims[0]) if not lims[0] is None else self.data.min()),
                (_np.datetime64(lims[1]) if not lims[1] is None else self.data.max()),
                )
            self.lims = lims
            self.capped = capped
...
#         def merge(self, other):
        ...
        def nice_interval(self, nTicks):
            lLim, uLim = self.lims
            to_s = lambda x: _np.datetime64(x, 's').astype('long')
            ...
            matches = abs(1. - _np.log(_np.array(bases)) / _np.log(nTicks))
            code = sorted(
        ...
        def nice_endpoints(self, code):
            ...
            start = _np.datetime64(lLims, npCode)
            stop = _np.datetime64(uLims, npCode) + 1
            return start, stop
        ...
        def nice_tickVals(self, nTicks = None, code = None, minor = True):
            ...
            if code == 'w':
                ...
                start = _np.datetime64(lLim, self._datetime_codesDict[code])
...
#             suffix = cls.latex_safe_date(suffix)
            return _np.array(labels), suffix

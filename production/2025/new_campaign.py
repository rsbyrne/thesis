...
class _JobManager:
    ...
    def get_jobs(self):
        ...
        if not len(jobs.shape) > 1:
            raise ValueError("Jobs are the wrong shape: " + str(jobs.shape))
        return jobs
...
if __name__ == '__main__':
    ...
    for key, param in sig.items():
        if param.kind is param.POSITIONAL_ONLY:
            ...
            parser.add_argument('-'+str(key[0]), '--'+str(key), type=param.annotation)
            arg_kinds[key] = 2
        else:
            raise ValueError(str(param.kind) + " not acceptable as a param kind.")

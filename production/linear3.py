...
JOBNAME, LOGPATH, COUNTER, *SLICE = sys.argv[1:]

with open(LOGPATH, mode = 'w') as logfile:

    log = get_logger(logfile)

    log(JOBNAME, LOGPATH, ', '.join(SLICE), COUNTER)

    import planetengine
    planetengine.set_global_anchor(JOBNAME, '.')
    ...
    etaDelta, f, aspect, H = get_job(dims, SLICE, COUNTER)

...
### Structuring infinite space

+++

We have repeatedly observed that hyperrealism gives us 'many ways' for a (real) number to be a number. This 'semantic space' now comes into its own, giving us all we need to structure the Pleroma into something more useful.

We have established that a Noum technically has a value in 'every direction', but that that value may be 'Void' (literally zero) in an arbitrary or even infinite number of dimensions. A Noum with Voids in it is effectively a Noum with two kinds of components: 'real' components (that can be embedded in real space without truncation) and 'hyperreal' components (that cannot). Equivalently, we can view every component as being a 'hyperreal' component, with the 'real' components merely being those whose non-standard part is zero.

It is here that the polytesimals we described earlier become useful. Let us stipulate that the non-standard parts of all components of a Noum *must be polytesimals*.

Actually, this is not the gross intervention it seems to be, because polytesimals - as we established earlier - must be derived from a given infinitesimal base, which prescribes their 'genre'. So the set of all Noums that comply with our new stipulation is identical to the set of all Noums without our stipulation because an arbitrary non-standard part can always be assigned by encasing it in a trivial polytesimal with the arbitrary infinitesimal as its base. In other words, Noums are 'polytesimal-valued' by default. The only thing that changes about Noums under this new stipulation is that Noums acquire a property of 'genre' - or rather, of 'genres' plural, since a Noum participates in the genre of every polytesimal. This creates two obvious classes of Noum: 'monogeneric' Noums, whose components' polytesimal parts all share the same genre, and 'polygeneric' Noums, whose components may be drawn from different genres. (Two polytesimals, components, or Noums that share the same genre or genres could be said to be 'cogeneric'.)

If every component of a Noum is viewed as a polytesimal in this way, and the polytesimals themselves can be viewed as points in $\mathbb{R}^\infty$ space as discussed earlier, then the 'real' components of any Noum (including any Void values) are just components whose polytesimal part is associated with the infinite real origin $(0, 0, 0...)$. If we consider only monogeneric Noums whose components are all unique (say 'non-duplicative'), then any Noum can be viewed as a pair of infinite vectors:

- A vector in $\mathbb{R}^\infty$ whose dimensions are unstructured - the 'real Pleroma' - representing the standard part of the Noum (its components without infinitesimal parts).
- A vector in $\mathbb{R}^\infty$ whose dimensions are structured, representing the non-standard part of the Noums - that is, the polytesimals.

Crucially, the components of these two vectors are associated with each other one-for-one. In short: *monogeneric non-duplicative Noums acquire the dimensional structure of their polytesimal parts*.



Let's draw up a quick example to illustrate what we mean. Taking the Roman alphabet to represent arbitrary real values and the Greek alphabet to represent arbitrary unique 'cogeneric' polytesimals, we can construct a pair of cogeneric Noums like this:

$$
\begin{aligned}
A &= \lgroup a+\alpha ,\space b+\beta ,\space c+\gamma \rgroup
\\
B &= \lgroup d+\alpha ,\space e+\beta ,\space f+\delta ,\space g+\epsilon \rgroup
\end{aligned}
$$

Interpreting the polytesimals as dimensions, we can see that we have a five-dimensional stucture (with dimensions 'labelled' $\alpha$ through $\epsilon$). Noum $A$ and Noum $B$ 'share' two dimensions $\alpha$ and $\beta$, but $A$ is inactive (that is to say, Void-valued or 'flat') in $\delta$ and $\epsilon$ while $B$ is inactive in $\gamma$. We can introduce new Noums to this space by simply reusing the relevant polytesimal parts.
...
Thus far, we have ignored the possibility of 'polygeneric' Noums - that is, Noums whose components are drawn from different genres.

The easiest way to contain the meaning of this is to view polygeneric Noums as being a 'blend' of monogeneric Noums. If Noums are viewed as vectors in the Pleroma, and if we adhere to a policy of assigning one dimension to every unique non-standard part in the whole set under consideration, then blending monogeneric Noums (or any Noums that do not 'share' any dimensions) simply creates a new Noum that varies in all the constituent dimensions. It is not conceptually different from blending a $x-y$ vector with a $z$ vector: the new displacement in $z$ does not affect the old displacement in $x$ and $y$.

Once we have prised out a polygeneric Noum's monogeneric 'planes', we can view the result as itself being a kind of vector in some even higher-dimensional space. For example, if Noum $A$ can be separated into three different planes - $A_\alpha$, $A_\beta$, and $A_\gamma$ - then we can imagine a sort of three-dimensional space where each axis encompasses a unique genre, with Noum $A$ giving a point in that space whose coordinates in each axis are themselves given as Noums. The resultant 'hyper-Noum' is a good example of the sort of 'bundled dimension' structure we discussed earlier, in which meaning is be packed into space in richly hierarchical ways.

+++

### Set-valued vectors and probability spaces

+++

Earlier, we casually pushed out of consideration any Noum with 'infinitesimally duplicative components' - that is, with components that share the same non-standard (polytesimal) part. Duplicate components make it impossible to achieve a unique one-to-one mapping between polycoefficient space and Pleromatic space: it would be akin to defining a point in Cartesian space with two $x$ values.

We could simply rule duplicative Noums illegal, but that would be waste of 'semantic space'. There is a richer interpretation we can provide - one that explicitly connects the Pleroma with a useful branch of mathematics.

Consider the Cartesian example. Imagine we are allowed to draw 'set-valued vectors' in this space - e.g. vectors where the components are sets of values instead of single values:
...
Since the contents of each component are unordered - and what's more, of different lengths - we can't naively pair them up and treat the whole vector as a group of vectors.

Each set by itself suggests a histogram: we could imagine 'bucketing' each set in some arbitrary way and then plotting the resulting histograms on each axis. Essentially, the information on each axis is being treated like a set of empirical measurements of a random variable, and the heatmap produced by the overlap is something reminiscent of a joint probability distribution.

Another approach is to take the Cartesian product of the two sets. If $m$ is the cardinality of the $x$ component and $n$ is the cardinality of the $y$ component, the result is a set of size $m \cdot n$ whose members are real vectors. Viewed that way, our set-valued vector can be interpreted as a cloud of points in $xy$ space. This cloud itself can then be viewed as a statistical object, with a mean (a centroid), a variance, and so on.

Transferring the same thinking from our example vectors back to Noums, we can see that 'duplicative Noums' can always be handled by decomposing them into clusters of non-duplicative Noums. Viewed from the opposite direction, it is now apparent that every Noum (even non-duplicative ones) should be treated in principle as a cluster of points, and thus the basic unit of analysis in the Pleroma is an essentially probabilistic proposition.

Broadening (or perhaps, 'enriching') our conception of Noums in this way is not without peril. We will have to take care when devising operators that act on Noums to ensure that we are always presumptively acting on clouds of points, without altering the intuitive vector-like behaviours we would expect when dealing with the 'degenerate' case of non-duplicative Noums (that is, 'clouds of only one point').

For example, if Noums are presumptively clouds, are we still able to define a strict 'distance' between them?
...
The solution relates to our previous observation about the 'histogram'-like nature of sets. To get the "Earth Mover's Distance", simply convert each set into a cumulative distribution function, plot them on the same axes, and take the area between the curves. In the case of degenerate (singleton) sets - i.e conventional vectors - this measure naturally collapses into a conventional distance, as we require. Because the method is founded in standard analysis, it works fine with infinite duplicates too. Thus we are comfortable declaring that a distance can always be found between any two Noums, no matter how 'exotic' they are.
...
When vector spaces are first introduced in standard maths curricula, a common starting point is the concept of the 'unit vectors', $i$ and $j$: objects with a real geometric meaning, but no semantic meaning, whose sole purpose is to endow other objects with structure.

In the Pleroma, our equivalent of unit vectors are what we will call 'Latent Noums', which are Noums whose standard form is the origin ($0, 0, 0...$). Noums clustered around zero have no semantic value (they do not indicate any degree of 'true' or 'false), but they do have geometric value. We have already seen how they can be used to indicate which dimensions are 'active' and which are 'inactive'. We will now show that they have the power to 'say' much more than that.

Firstly, let's be clear. What we are calling a Latent Noum is one whose components each have a standard (real) part equal to zero. Given that we already decided to treat all non-standard parts as polytesimals, it is plain that a (monogeneric) Latent Noum is a multiset of (cogeneric) polytesimals. Since every polytesimal can be interpreted as a unique point in polycoefficient space ($\mathbb{R}^{\infty}$), a set of such points can be viewed as a particle cloud in real space.

It should already be clear, then, how a Latent Noum can be seen as representing a 'structure', since - to be baldly literal - there is nothing stopping us from forming a swarm of points into the shape of the Taj Mahal if we choose. Of course, we are interested in more austere structures, but the same principle applies. Symmetries in polycoefficient space can always be transformed into symmetries in the Pleroma as long as we remain in the same infinitesimal 'genus'.
...
We already know how to support 'flat' dimensional structures using polytesimals. Taking $L$ as an arbitrary 'Latent Noum' of our choosing and Greek letters as (unique) polytesimals, we can simply declare:

$$
L = \lgroup \alpha, \beta, \gamma, \delta, ..., \omega \rgroup
$$

This is akin to declaring an 'alphabet' in formal grammar, or a symbol set in abstract algebra - or indeed, a set of 'unit types' in computer science.

A real vector $V$ can embed itself into the Pleroma as a Noum $N$ simply by element-wise addition with $L$:

$$
V + L = N
\\
(V_1, V_2, ..., V_n) + \lgroup \alpha, \beta, \gamma, \delta, ..., \omega \rgroup = \lgroup V_1 + \alpha ,\space V_2 + \beta \space... \space V_n + \omega \rgroup
$$

Now, thus far, we have treated $\alpha$, $\beta$, and the rest as merely 'arbitrary polytesimals'. That is valid, as far as it goes: it is given that every unique polytesimal 'tags' a unique dimension in the Pleroma from the perspective of a cogeneric Noum. However, it ignores that (cogeneric) polytesimals themselves have implicit relationships with each other through their embedding in real space. For example, we could give that:

$$
\begin{aligned}
\alpha &= a \epsilon^{1} + b \epsilon^{2}
\\
\beta &= c \epsilon^{1} + d \epsilon^{3}
\end{aligned}
$$

Clearly, $\alpha$ and $\beta$ are unique. Yet they have two important things in common: they are both 'two dimensional' in polycoefficient space, and they both extend into the same first dimension (albeit in different magnitudes). We can see that the pair mutually cohabit a shared three-dimensional space.

What can we do with this very simple symmetry? Well, say we have a Noum $A$ with an $\alpha$ component, but no $\beta$ component:

$$
A = \lgroup a + \alpha \rgroup
$$

Without changing the value of $a$ at all, we can shift $A$ from dimension $\alpha$ to dimension $\beta$ by adding the difference between them - which is itself a polytesimal, $\Delta$, the vector pointing from $\alpha$ to $\beta$ in polycoefficient space:

$$
\Delta = \beta - \alpha = (c-a) \epsilon - b \epsilon^{2} + d \epsilon^{3}
\\
B = A + \Delta = \lgroup a + \alpha + \Delta \rgroup = \lgroup a + \beta \rgroup
$$

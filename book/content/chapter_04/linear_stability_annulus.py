...
def compute_annulus_ra_cr(f, m, N=50, regime="basal"):
    ...
    # We explicitly zero out the boundary rows across all columns in A and B 
    # before injecting the identity constraints. This prevents spurious coupling
    # and keeps the matrix rank stable.

    # 1. Psi = 0 (Impenetrable walls at r=ro [row 0] and r=ri [row N])
    A[0, :] = 0;   A[0, 0] = 1;   B[0, :] = 0
    A[N, :] = 0;   A[N, N] = 1;   B[N, :] = 0

    # 2. Phi + (2/r)*Psi' = 0 (Free-Slip zero tangential stress)
    # Outer boundary (row M)
    A[M, :] = 0;   B[M, :] = 0
    ...
    # 3. Theta Boundary Conditions (Rows 2*M and 2*M+N)
    # Outer boundary: Always Isothermal (Theta = 0)
    A[2*M, :] = 0; A[2*M, 2*M] = 1; B[2*M, :] = 0

    # Inner boundary: Depends on the heating regime
    A[2*M+N, :] = 0; B[2*M+N, :] = 0
    if regime == "internal":
        # True Insulated bottom: dTheta/dr = 0 at r=ri
        row_bot_theta = np.zeros(3*M)
        row_bot_theta[2*M:3*M] = Dr[-1, :]
        A[2*M+N, :] = row_bot_theta / np.max(np.abs(row_bot_theta))
    elif regime == "basal":
        # Isothermal bottom: Theta = 0 at r=ri
...
# Lord Rayleigh 1916 benchmark for purely basally heated:
assert (val := round(compute_annulus_ra_cr(f=0.99, m=221, regime="basal"), 1)) == 657.5, val

# Roberts 1967 benchmark for purely internally heated:
